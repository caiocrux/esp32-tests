...
esp_netif_t *wifi_app_get_sta_netif(void) { return esp_netif_sta; }
...
static void wifi_app_soft_ap_config(void) {
  ...
  inet_pton(
      AF_INET, WIFI_AP_IP_ADDR,
      &ap_ip_info.ip); ///> Assign access point's static IP, GW, and netmask
  inet_pton(AF_INET, WIFI_AP_IP_GTW, &ap_ip_info.gw);
  ...
  ESP_ERROR_CHECK(esp_netif_dhcps_start(
      esp_netif_ap)); ///> Start the AP DHCP server (for connecting stations
...
static void wifi_app_task(void *pvParameters) {
  ...
  for (;;) {
    if (xQueueReceive(wifi_app_queue_handle, &msg, portMAX_DELAY)) {
      ...
      default:
        ESP_LOGW(TAG, "Unhandled WIFI event %d", msg.msgID);
